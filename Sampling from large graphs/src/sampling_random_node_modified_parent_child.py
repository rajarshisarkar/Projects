# To Run this script: python sampling_random_node_modified_parent_child.py <year>
...
plt.suptitle('Average Clustering Coefficient Bias vs Random node sample (via modified random node sampling) % plot till year '+str(curyear), fontsize=10)
plt.xlabel('Random node sample (via modified random node sampling) %')
plt.ylabel('Average Clustering Coefficient Bias')
plt.savefig('../graphs/Average Clustering Coefficient Bias vs Random node sample (via modified random node sampling) % plot till year '+str(curyear)+'.png')
plt.close()

plt.figure()
plt.xlim(0,60)
plt.plot(samplepercents100,baisbyavgclusteringcoefofnetwork,'r^-')
plt.suptitle('Average Bias/Average Clustering Coefficient vs Random node sample (via modified random node sampling) % plot till year '+str(curyear), fontsize=10)
plt.xlabel('Random node sample (via modified random node sampling) %')
plt.ylabel('Average Bias/Average Clustering Coefficient')
plt.savefig('../graphs/Average Bias by Average Clustering Coefficient vs Random node sample (via modified random node sampling) % plot till year '+str(curyear)+'.png')
plt.close()

plt.figure()
plt.xlim(0,60)
plt.plot(samplepercents100,avgdvalues,'r^-')
plt.suptitle('K-S statistic value for degree distribution vs Random node sample (via modified random node sampling) % plot till year '+str(curyear), fontsize=10)
plt.xlabel('Random node sample (via modified random node sampling) %')
plt.ylabel('K-S statistic value for degree distribution')
plt.savefig('../graphs/K-S statistic value for degree distribution vs Random node sample (via modified random node sampling) % plot till year '+str(curyear)+'.png')
plt.close()
