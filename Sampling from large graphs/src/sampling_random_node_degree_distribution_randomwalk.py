# To Run this script: python sampling_random_node_degree_distribution_randomwalk.py <year>
...
for samplepercent in samplepercents:
	...
	for iteration in range(0,n):
		...
		for x in range(curyear, curyear+1):
			...
			line2, = plt.plot(values2,newhist2,'ro',label='Random node sample network (via randomwalk)')
			...
			plt.suptitle('Degree Distribution plot for '+str(int(samplepercent*100))+'% random node sample (via randomwalk) till year '+str(curyear), fontsize=12)
			plt.xlabel('Degree')
			plt.ylabel('Fraction of nodes')
			plt.savefig('../graphs/Degree Distribution plot for '+str(int(samplepercent*100))+'% random node sampling (via random walk) till year '+str(curyear)+'.png')
			plt.close()
	print
