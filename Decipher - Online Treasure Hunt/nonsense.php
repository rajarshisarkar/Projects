...
<title>Q3</title><script>
					function open_win()
					{
					window.open("nonsense.php")
					}
					</script>
					
</head>
<?php
if($_SESSION['normaluser']=='normaluser' && $_SESSION['level']==3)
{
...
<hr color="#CC0000">
<p><strong> <center>
  <font size="5">Question 3</strong><br></font>
  </h2><center>';
	include ("connect.php");
	$result=mysqli_query($con,"select qdesc,lifeline1,lifeline2,imagename,answer from questions where qno=3");
	$rows=mysqli_fetch_array($result);					
	echo ''.$rows[0].'';

echo'<form name="q3" action="nonsense.php" method="post">
<br>Answer : <input type="text" name="q3" size="50"><br>
<input type="submit" value="Submit" name="submit">
<br><input type="submit" value="Lifeline 1 ( -1 point )" name="life3_1" onclick="open_win()">&nbsp;&nbsp;&nbsp; 
<input type="submit" value="Lifeline 2 ( -1 point )" name="life3_2" onclick="open_win()"><br>

</form>';
echo '<br><img src="photos/'.$rows[3].'.jpg"></img>'; 
		if(isset($_POST['life3_1']))
		{
		include ("connect.php");
		$username=$_SESSION['username'];
		$result=mysqli_query($con,"select life3_1 from users where name='$username'");
		$rows=mysqli_fetch_array($result);
					if($rows[0]==0)
					{
					$username=$_SESSION['username'];
					$result=mysqli_query($con,"update users set life3_1=1 where name='$username'");
					$result=mysqli_query($con,"update users set score=(score-1) where name='$username'");
					}
					
					$result=mysqli_query($con,"select lifeline1 from questions where qno=3");
					$rows=mysqli_fetch_array($result);
					header('Location: '.$rows[0].'');
		}
		
		if(isset($_POST['life3_2']))
		{
		include ("connect.php");
		$username=$_SESSION['username'];
		$result=mysqli_query($con,"select life3_2 from users where name='$username'");
		$rows=mysqli_fetch_array($result);
					if($rows[0]==0)
					{
					$username=$_SESSION['username'];
					$result=mysqli_query($con,"update users set life3_2=1 where name='$username'");
					$result=mysqli_query($con,"update users set score=(score-1) where name='$username'");
					}
					
					$result=mysqli_query($con,"select lifeline2 from questions where qno=3");
					$rows=mysqli_fetch_array($result);
					header('Location: '.$rows[0].'');
		}
...
';
		...
		if($_POST['q3']==$rows[4])		
			...
			else header('Location: nonsense.php');
		}
}
