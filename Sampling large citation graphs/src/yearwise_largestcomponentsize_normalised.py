# To Run this script: python yearwise_largestcomponentsize_normalised.py
...
for x in range(1975, 2005+1):
	...
	largestcomponentsize.append(float(len(largestcomponent))/float(G.number_of_nodes()))
	year.append(x)
...
plt.title('Normalised Largest Component Size vs Year plot')
plt.xlabel('Year')
plt.ylabel('Normalised Largest Component Size')
plt.savefig('../graphs/Normalised Largest Component Size vs Year plot.png')
plt.close()
